...
/* Portable printf for embedded/newlib: avoid %zu and %f (often not in nano). */
#define FMT_SIZE "lu"
#define CAST_SIZE(x) ((unsigned long)(x))
#define FMT_TIME "lu"
#define CAST_TIME(x) ((unsigned long)((x) * 1000))  /* milliseconds */
...
void MergeSort(Test array[], const size_t array_count, const Comparison compare) {
	Test *buffer = (Test *)malloc(((array_count + 1)/2) * sizeof(Test));
	if (!buffer) {
		printf("MergeSort: malloc failed for %" FMT_SIZE " elements\n", CAST_SIZE((array_count + 1)/2));
		return;
	}
...
/* Smaller sizes for embedded/simulator (limited heap and stack); use 1500000 on host if desired.
 * If exit 7 persists, try -DWIKISORT_MAX_SIZE=2048 -DWIKISORT_STEP=512 (fewer, smaller steps). */
#ifndef WIKISORT_MAX_SIZE
#define WIKISORT_MAX_SIZE 4096
#endif
#ifndef WIKISORT_STEP
#define WIKISORT_STEP 1024
#endif

int main() {
	size_t total, index;
	double total_time, total_time1, total_time2;
	const size_t max_size = WIKISORT_MAX_SIZE;
	...
	for (total = 0; total < max_size; total += WIKISORT_STEP) {
		double time1, time2;
		...
		if (time1 <= 0.0) {
			printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (n/a)\n", CAST_TIME(time1), CAST_TIME(time2));
		} else if (time1 >= time2)
			printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (%lu%% as fast)\n", CAST_TIME(time1), CAST_TIME(time2), (unsigned long)(time2/time1 * 100.0));
		else
			printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (%lu%% faster)\n", CAST_TIME(time1), CAST_TIME(time2), (unsigned long)(time2/time1 * 100.0 - 100.0));
		
		#if PROFILE
			if (compares1 <= compares2)
				printf("WikiSort: %" FMT_SIZE " compares, MergeSort: %" FMT_SIZE " compares (%lu%% as many)\n", CAST_SIZE(compares1), CAST_SIZE(compares2), (unsigned long)(compares1 * 100.0/compares2));
			else
				printf("WikiSort: %" FMT_SIZE " compares, MergeSort: %" FMT_SIZE " compares (%lu%% more)\n", CAST_SIZE(compares1), CAST_SIZE(compares2), (unsigned long)(compares1 * 100.0/compares2 - 100.0));
		#endif
	...
	printf("tests completed in %" FMT_TIME " ms\n", CAST_TIME(total_time));
	if (total_time1 <= 0.0) {
		printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (n/a)\n", CAST_TIME(total_time1), CAST_TIME(total_time2));
	} else if (total_time1 >= total_time2)
		printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (%lu%% as fast)\n", CAST_TIME(total_time1), CAST_TIME(total_time2), (unsigned long)(total_time2/total_time1 * 100.0));
	else
		printf("WikiSort: %" FMT_TIME " ms, MergeSort: %" FMT_TIME " ms (%lu%% faster)\n", CAST_TIME(total_time1), CAST_TIME(total_time2), (unsigned long)(total_time2/total_time1 * 100.0 - 100.0));
	
	#if PROFILE
		if (total_compares1 <= total_compares2)
			printf("WikiSort: %" FMT_SIZE " compares, MergeSort: %" FMT_SIZE " compares (%lu%% as many)\n", CAST_SIZE(total_compares1), CAST_SIZE(total_compares2), (unsigned long)(total_compares1 * 100.0/total_compares2));
		else
			printf("WikiSort: %" FMT_SIZE " compares, MergeSort: %" FMT_SIZE " compares (%lu%% more)\n", CAST_SIZE(total_compares1), CAST_SIZE(total_compares2), (unsigned long)(total_compares1 * 100.0/total_compares2 - 100.0));
	#endif
